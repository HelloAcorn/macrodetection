...
def MonsterData(dbname, blockname):
    ...
    for i in range(blocks):
        cursor.execute("select trim(MONSTER) from %s where block = '%d'" % (dbname, i))
        data = cursor.fetchall()
        data = np.array(data, dtype=int)
        if len(data) == 0:
            continue
...
for i in range(len(SEEDS)):
    ...
    with torch.no_grad():        # 평가 과정에는 미분이 필요없음
        ...
        Result_arr.append(predict_percent)
        print('\n테스트 데이터에서 예측 정확도: {}/{} ({:.0f}%)\n'.format(correct, data_num, predict_percent))

print("epoch = {}, copynode = {}".format(n_epochs, copynode))
for i in range(len(Result_arr)):
    print(i+1,"번째 시드에서의 테스트 정확도: ",Result_arr[i])
