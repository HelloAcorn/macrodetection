...
#11/06 현재 사람같은 사냥 하는 민윤홍의 db
def update_DB():
    while WORKS_PROGRAM == True:
        keyblock = 0
        time.sleep(30)
        cursor.execute("select * from good_keyblock")
        data = cursor.fetchall()
        keyblock = data[0][0]
        #good_keyblock
        cursor.execute("update good_keyblock set count = count+1")
        
        
        for i in range(len(UPLOAD_DB_LISTED)):
            #스킬키, x좌표, y좌표, 몬스터 처치수 , real_yoonhong
            cursor.execute("INSERT INTO real_yoonhong VALUES ('%s','%d','%d','%d','%f', '%d')"
                           %(UPLOAD_DB_LISTED[i][0], UPLOAD_DB_LISTED[i][1],
                             UPLOAD_DB_LISTED[i][2], UPLOAD_DB_LISTED[i][3],
                             UPLOAD_DB_LISTED[i][4], keyblock))

        print(UPLOAD_DB_LISTED)
        UPLOAD_DB_LISTED.clear()
